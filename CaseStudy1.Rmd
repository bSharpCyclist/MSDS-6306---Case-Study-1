...
*  Statistical analysis on ABV and the relationship between ABV and IBU
...
brewPlot <-ggplot(data=brewTable, aes(x=State, y=Count, fill=State))
brewPlot +
...
abvPlot <-ggplot(data=medianABV, aes(x=State, y=median))
abvPlot +
  geom_bar(stat="identity") +
  ylab("Median ABV") +
  xlab("State") +
...
ibuPlot <-ggplot(data=medianIBU, aes(x=State, y=median))
ibuPlot +
  geom_bar(stat="identity") +
  ylab("Median IBU") +
  xlab("State") +
...
Take notice that South Dakota didn't have an IBU value ...
...
# or do something like this to remvoe NAs
summary(mergedData$ABV[!is.na(mergedData$ABV)])
```

Need to include summary statistics. Just general stuff, question is vague.

```{r fig.align='center', fig.width=11}
ggplot(mergedData[!(is.na(mergedData$IBU)|is.na(mergedData$ABV)),], aes(x=IBU, y=ABV)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = TRUE)
summary(lm(mergedData$ABV ~ mergedData$IBU))
```

### Is there an apparent relationship between the bitterness of the beer and its alcoholic content? Draw a scatter plot.

Look at more statistics options and come up with a good conclusion
